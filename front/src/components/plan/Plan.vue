<template>
  <v-card :elevation="elevation" class="border duration-300 rounded-[12px] px-2" @mouseover="elevation = 3" @mouseleave="elevation = 0">
    ...
    <div v-if="plan" class="relative">
      <v-card-text class="ga-3 flex flex-col">
        ...
        <div class="flex ga-2 items-center">
          ...
          <div>{{ t('plans.lifespan') }}</div>
          <div v-if="plan?.lifespan">{{ plan?.lifespan / (24 * 60 * 60) }}</div>
          <v-chip v-else color="primary" label density="compact" size="small">
            <v-icon>mdi-all-inclusive</v-icon>
          </v-chip>
          <div v-if="plan?.lifespan">{{ t('plans.days') }}</div>
        </div>
      ...
      <div class="absolute right-0 bottom-0 mb-1 mr-3" v-if="!readonly">
        <v-tooltip :text="fold ? t('plans.expand') : t('plans.collapse')">
          <template #activator="{ props }">
            <v-btn
              :icon="fold ? 'mdi-chevron-down' : 'mdi-chevron-up'"
              variant="text"
              v-bind="props"
              color="primary"
              @click="fold = !fold"
    ...
    <v-slide-y-reverse-transition v-if="!readonly">
      <div v-show="!fold">
        <v-divider opacity="1" />
        <v-card-actions>
          <v-btn :color="plan?.enable ? 'primary' : undefined" prepend-icon="mdi-creation-outline" @click="genKey" :disable="!plan?.enable">
            {{ t('plans.generateKeys') }}
          </v-btn>
          <v-spacer />
          <v-btn prepend-icon="mdi-square-edit-outline" @click="onEditClick" :disable="!plan?.enable" :color="plan?.enable ? 'primary' : undefined">
            {{ t('common.edit') }}
          </v-btn>
    ...
    <GenPlanKeysDialog v-model="showGenKeyDialog" :id="plan?.id" />
  </v-card>
</template>
